...
let animalesMayus = animales.map(names =>{

    return names.toUpperCase()
})
 console.log(animalesMayus) // ["VERTÍN, BIGOTES"]
...
let pacientesVisitados = pacientes.map(fecha => fecha.ultimaVisita = new Date().toLocaleString()
...
let pacientesAvisos = pacientes.map((item)=>{
    return{
        ...item,avisos:"paga mamon" && item.ultimaVisitaPagada == true ? "" : "paga mamon"
    }

})
console.log(pacientesAvisos)
